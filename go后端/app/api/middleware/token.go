...
func AnalyseToken(tokenConfig commonmodel.TokenConfig, dbContext *model.DBContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !strings.HasPrefix(rawAccessToken, "Bearer ") {
			...
			logger = newlog.AddGateWayInfo(newlog.AddError(logger, fmt.Errorf("Lack Access Token"), newerror.CodeMissingParam), http.StatusUnauthorized, 0, c.ClientIP(), c.FullPath())
		...
		if deviceID != c.GetHeader("X-Device-ID") {
			...
			logger = newlog.AddError(logger, fmt.Errorf("Device ID Error In Token"), newerror.CodeAccessTokenInvalid)
			logger = newlog.AddGateWayInfo(logger, http.StatusUnauthorized, userID, c.ClientIP(), c.FullPath())
			newlog.SetGinLog(c, logger, "Analyse Token", newerror.LevelInfo)
			c.Abort()
			return
		}
